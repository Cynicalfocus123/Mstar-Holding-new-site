...
- Fixed the shared Business observer, which paused company videos on every exit callback during scrolling. It now prepares and starts nearby videos but never pauses them merely because a section leaves the viewport.
- Every company video now starts with `data-src` and `preload="none"`; one 1600px shared preparation observer attaches each source and calls `load()` once. Initialized videos retain their DOM node, source, current time, loop, and playback while the visitor remains on Business.
- The guarded playback helper enforces muted/default-muted/inline/loop before source assignment, waits for usable media data, safely handles promise rejections, retries on visible `pageshow`/tab return/layout changes, and attempts one restart after an unexpected visible-page pause. The poster stays on top until `playing` fires.
- Production build, lint, test, weight audit, zero-missing/zero-unused deploy audit, and ZIP SHA-256 extraction parity pass. After the balanced Finance-video replacement, clean `dist/` is 51,229,754 bytes across 152 files; the refreshed canonical ZIP is 50,445,837 bytes with 187 file/directory entries.

## 2026-07-28 Premier Escrow Services Finance Video

- Rebalanced only `videos/business-premier-escrow-services-v3.mp4` with the same 720x1280, 60 FPS, silent H.264/yuv420p, fast-start profile at CRF 25/preset slow. It is now 3,190,726 bytes (66.71% below the 9,584,323-byte source); no page code, layout, poster, or other video changed.
...
- All Business company videos use `data-src` and `preload="none"`. The shared Business observer attaches each source and calls `load()` 1600px before view; it keeps the poster until playback begins and does not pause initialized videos when they scroll out of view.
- `build:hostinger`, lint, test, weight audit, FFprobe, full decode, reference audit, and ZIP parity passed for the Finance-video delivery revision. That revision's clean `dist/` was 52,098,785 bytes across 152 files; its direct-root ZIP was 51,315,243 bytes with SHA-256 extraction parity.
...
- Airsoft alone sets `imageFit: "contain"`: its full portrait artwork is visible in the unchanged equal-height Home card on a matching gray background and at its natural portrait ratio on the detail page. Other News articles retain their existing responsive crop behavior.
- On News & Media only, the Airsoft card now uses the standard full-card crop at desktop, tablet, and mobile, focused at the top group photo like the Finance card. Its Home card and detail image remain full-image, and no other article is changed.
...
- Build, lint, tests, weight audit, reference cleanup, static SEO/path checks, and ZIP extraction parity pass. Clean `dist/`: 47,965,119 bytes / 150 files; ZIP: 47,173,990 bytes / 150 direct-root entries with SHA-256 extraction parity.
...
- Increased the visible tablet-only offsets for Mstar Property Development, SeniorCaring, American Buying Service, Mstar Defense, Mstar Technology, and HIZOZ so the GitHub Pages tablet layout moves clearly in the marked arrow directions.
- Preserved all unmentioned logo positions, mobile behavior, desktop behavior, wheel design/colors, center hub, company-to-sector mapping, heading text, page background, original logo artwork, and transparent no-card logo rendering.
- Honored the request not to open localhost, not to open a local IP, and not to create a ZIP.

## 2026-07-09 Business Ecosystem Tablet Targeted Logo Nudges

- Updated only the standalone Global Presence Business Ecosystem tablet breakpoint from 768px through 1199px.
- Adjusted only HIZOZ, Mstar Technology, SeniorCaring, American Buying Service, and Mstar Defense: Technology logos moved right, SeniorCaring and American Buying Service backed away from their slice edges, and Mstar Defense moved closer to the Defense slice.
- Preserved all unmentioned logo positions, mobile behavior, desktop behavior, wheel design/colors, center hub, company-to-sector mapping, heading text, page background, original logo artwork, and transparent no-card logo rendering.
- Honored the request not to open localhost, not to open a local IP, and not to create a ZIP.
...
- Updated only the standalone Global Presence Business Ecosystem tablet breakpoint from 768px through 1199px.
- Adjusted only the named tablet logo wrappers: Mstar Property Development, SeniorCaring, American Buying Service, Mstar Defense, Mstar Technology, and One Tastes.
- Preserved all unmentioned logo positions, mobile behavior, desktop behavior, wheel design/colors, center hub, company-to-sector mapping, heading text, page background, original logo artwork, and transparent no-card logo rendering.
- Honored the request not to open localhost, not to open a local IP, and not to create a ZIP.
...
- Updated only the standalone Global Presence Business Ecosystem tablet breakpoint from 768px through 1199px.
- Pulled the Real Estate, Food & Hospitality, Technology, and Defense logo groups closer to their matching wheel sectors, using the closer Entertainment and Import / Export orbit spacing as the visual reference.
- Preserved mobile behavior, desktop behavior, wheel design/colors, center hub, company-to-sector mapping, heading text, page background, original logo artwork, and transparent no-card logo rendering.
- Honored the request not to open localhost, not to open a local IP, and not to create a ZIP.
...
- Updated only the standalone Global Presence Business Ecosystem tablet breakpoint from 768px through 1199px.
- Reworked tablet logo orbit spacing to follow the desktop outside-wheel logic with a centered wheel, larger perimeter gap, expanded tablet layout height, and logo groups pushed farther away from the sector edges.
- Preserved mobile in-slice logo behavior, desktop orbit behavior, sector colors, wheel slice design, center hub, company-to-sector mapping, original logo artwork, heading text, page background, and no-card/no-box logo rendering.
- Honored the request not to open localhost, not to open a local IP, and not to create a ZIP.
...
- Follow-up Technology logo spacing correction moved only HIZOZ farther right to create more separation from Mstar Technology while preserving Mstar Technology's placement.
...
- Tightened the homepage stacked mobile/tablet Regional Expansion layout so the curved image touches the theme-row top line instead of floating above the boxes.
...
- Added a new standalone `global-presence/` page for the requested Global Presence stat-card and globe experience.
- Connected every header and shared footer Global Presence link to the new page using correct relative paths from root, one-level pages, and two-level article/governance pages.
- Added the new page to the Vite build input list so `dist/global-presence/index.html` is generated for Git/source and Hostinger/public output.
- Implemented a dedicated canvas Earth/globe renderer for the new page using existing Natural Earth continent path data, so the globe shows actual landmasses with teal operational markers and gold partnership arcs.
- Preserved About page text, President Message, About collage, original About Global Presence map, Built Across Industries, header/footer visual design, and unrelated pages/assets.
...
- Replaced the old Antonia Espada board entry and asset reference with Antonio Pereira everywhere on the Board of Directors page.
- Re-exported Paul Leung and Antonio Pereira as cleaned transparent lossless WebP assets with only border-connected neutral/checker/white background pixels removed.
...
- Added board members in the requested order: Jakapun Viwatkurkul, Mike Zhao, Jeffrey Qiu, Rayyan Al-Assad, Paul Leung, Antonio Pereira, and Ben King.
...
- Built production and Hostinger/root output, then refreshed the existing `mstar-latest-changes-deploy.zip` in place without creating a new ZIP filename.
- ZIP verification: 55,756,748 bytes, 111 entries, no wrapper folder, no source/development files, no Windows backslash paths, and the Board page plus six board WebP assets present.
- Verification passed: `cmd /c npm.cmd run build`, `cmd /c npm.cmd run lint`, `cmd /c npm.cmd test`, `cmd /c npm.cmd run build:hostinger`, and `cmd /c npm.cmd run weight:audit`.
